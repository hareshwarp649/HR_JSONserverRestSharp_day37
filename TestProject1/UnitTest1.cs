    ...
    
        {
            ...
            */
            ...
            public void onCallingGETApi_ReturnEmployeeList()
            {
                ...
                List<DataEmpPay> dataResponse = JsonConvert.DeserializeObject<List<DataEmpPay>>(response.Content);
                Assert.AreEqual(7, dataResponse.Count);

                foreach (DataEmpPay e in dataResponse)
                {
            ...
            public void OnCallingPostAPI_ReturnEmployeeObject()
            {
                ...
                request.AddBody(new DataEmpPay
                {
                    id = 8,
                    name = "Clark",
                    salary = "15000"
                ...
                DataEmpPay dataResponse = JsonConvert.DeserializeObject<DataEmpPay>(response.Content);
            ...
            {
                // Arrange
                List<DataEmpPay> employeeList = new List<DataEmpPay>();
                employeeList.Add(new DataEmpPay { name = "Vinaya", salary = "15000" });
                employeeList.Add(new DataEmpPay { name = "Ajaya kumar", salary = "7000" });
                employeeList.Add(new DataEmpPay { name = "Powan", salary = "9000" });
                employeeList.Add(new DataEmpPay { name = "Swathi", salary = "12000" });
                ...
                foreach (var emp in employeeList)
                {
                    ...
                    //Assert
                    Assert.AreEqual(response.StatusCode, HttpStatusCode.Created);
                    DataEmpPay employee = JsonConvert.DeserializeObject<DataEmpPay>(response.Content);
